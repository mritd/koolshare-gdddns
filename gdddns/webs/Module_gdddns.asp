							...
							<table width="760px" border="0" cellpadding="5" cellspacing="0" bordercolor="#6b8fa3" class="FormTitle" id="FormTitle">
								<tr>
									<td bgcolor="#4D595D" colspan="3" valign="top">
										...
										<div class="SimpleNote" id="head_illustrate"><i></i><em>Godaddy DDNS</em>是一款基于Godaddy 云解析的私人 DDNS 解决方案。<a href='http://koolshare.cn/thread-64703-1-1.html' target='_blank'><i>&nbsp;&nbsp;<u>点击查看插件详情</u></i></a></div>
                						<table style="margin:20px 0px 0px 0px;" width="100%" border="1" align="center" cellpadding="4" cellspacing="0" bordercolor="#6b8fa3" class="FormTable">
                						    ...
                						    <tr id="last_act_tr">
                						        <th>上次运行</th>
                						        <td>
                						            <% dbus_get_def("godddns_last_act", "--"); %>
                						        </td>
                						    </tr>
                						    <tr id="ak_tr">
                						        <th>Godaddy key</th>
                						        <td>
                						            <input type="text" id="godddns_ak" value="<% dbus_get_def("godddns_ak", ""); %>" class="input_ss_table">
                						        </td>
                						    </tr>
                						    <tr id="sk_tr">
                						        <th>Godaddy secret</th>
                						        <td><input type="password" id="godddns_sk" value="<% dbus_get_def("godddns_sk", ""); %>" class="input_ss_table"></td>
                						    </tr>
                						    <tr id="interval_tr">
                						        <th>检查周期</th>
                						        <td><input type="text" style="width: 2.5em" id="godddns_interval" value="<% dbus_get_def("godddns_interval", "120"); %>" class="input_ss_table">s</td>
                						    </tr>
                						    <tr id="name_tr">
                						        <th>域名</th>
                						        <td>
                						            <input type="text" style="width: 4em" id="godddns_name" placeholder="子域名" value="<% dbus_get_def("godddns_name", "home"); %>" class="input_ss_table"
                						            >.<input type="text"  id="godddns_domain" placeholder="主域名" value="<% dbus_get_def("godddns_domain", "example.com"); %>" class="input_ss_table">
                						        </td>
                						    </tr>
                						    <tr id="dns_tr">
                						        <th title="查询域名当前IP时使用的DNS解析服务器，默认为 Godaddy DNS">DNS服务器(?)</th>
                						        <td><input id="godddns_dns" class="input_ss_table" value="<% dbus_get_def("godddns_dns", "NS05.DOMAINCONTROL.COM"); %>"></td>
                						    </tr>
                						    <tr id="curl_tr">
                						        <th title="可自行修改命令行，以获得正确的公网IP。如添加 '--interface vlan2' 以指定多播情况下的端口支持">获得IP命令(?)</th>
                						        <td><textarea id="godddns_curl" class="input_ss_table" style="width: 94%; height: 2.4em"><% dbus_get_def("godddns_curl", "curl -s whatismyip.akamai.com"); %></textarea></td>
                						    </tr>
                						    <tr id="ttl_tr">
                						        <th title="设置解析TTL，默认10分钟，免费版的范围是600-86400">TTL(?)</th>
                						        <td><input id="godddns_ttl" style="width: 4.5em" class="input_ss_table" value="<% dbus_get_def("godddns_ttl", "600"); %>">s (1~86400)</td>
                						    </tr>
                						</table>
                						<div class="apply_gen">
...
$(function () {
    show_menu(menu_hook);
    var enable = "<% dbus_get_def("godddns_enable", "0"); %>";
    ...
    $('.button_gen').click(function () {
        ...
        posting = true; // save
		var data = {
			godddns_enable: $('#switch').prop('checked') | 0,
			action_mode: ' Refresh ',
			current_page: 'Module_godddns.asp',
			next_page: 'Module_godddns.asp',
			SystemCmd: 'godddns_config.sh'
		};
...
function menu_hook(title, tab) {
	if(enable_ss == "1" && enable_soft == "1"){
		tabtitle[17] = new Array("", "godddns");
		tablink[17] = new Array("", "Module_godddns.asp");
	}else{
		tabtitle[16] = new Array("", "godddns");
		tablink[16] = new Array("", "Module_godddns.asp");
	}
}
